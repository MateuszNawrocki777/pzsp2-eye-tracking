...
class TestControllerTest {
    ...
    void setUp() {
        ...
        other = userAccountRepository.save(new UserAccount(UUID.randomUUID(), "other@test.local", "pw", UserRole.USER));
    }
    ...
    void getAllTests_unauthenticated_returns401() throws Exception {
        mockMvc.perform(get("/api/tests").accept(MediaType.APPLICATION_JSON))
    ...
    void downloadFile_returnsFileContents() throws Exception {
        ...
        mockMvc.perform(get("/api/tests/files/" + m.getMaterialId()))
                .andExpect(status().isOk())
    ...
    void deleteTest_nonOwner_forbidden_owner_allowed() throws Exception {
        ...
        mockMvc.perform(delete("/api/tests/" + study.getStudyId()).header("Authorization", bearer(other)))
                .andExpect(status().isForbidden());

        mockMvc.perform(delete("/api/tests/" + study.getStudyId()).header("Authorization", bearer(owner)))
                .andExpect(status().isNoContent());
