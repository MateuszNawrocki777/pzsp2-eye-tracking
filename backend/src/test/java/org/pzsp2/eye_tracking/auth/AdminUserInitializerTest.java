...
@SuppressWarnings("null") class AdminUserInitializerTest {
    ...
    @Test void shouldCreateAdminWhenMissing() {
    ...
    @Test void shouldUpdatePasswordWhenAdminExistsWithDifferentSecret() {
        given(adminUserProperties.email()).willReturn("admin@example.com");
        given(adminUserProperties.password()).willReturn("NewPass!");

        UserAccount existing = new UserAccount(UUID.randomUUID(), "admin@example.com", "old-hash",
                        UserRole.ADMIN);
    ...
    @Test void shouldDoNothingWhenPasswordMatches() {
        given(adminUserProperties.email()).willReturn("admin@example.com");
        given(adminUserProperties.password()).willReturn("SamePass!");

        UserAccount existing = new UserAccount(UUID.randomUUID(), "admin@example.com", "hash",
                        UserRole.ADMIN);
    ...
    @Test void shouldSkipSetupWhenUserExistsButNotAdmin() {
        given(adminUserProperties.email()).willReturn("user@example.com");

        UserAccount existing = new UserAccount(UUID.randomUUID(), "user@example.com", "hash",
                        UserRole.USER);
