...
@ExtendWith(MockitoExtension.class) class JwtAuthenticationFilterTest {
    ...
    @Test void doFilter_noAuthHeader_continuesChain() throws Exception {
    ...
    @Test void doFilter_badAuthHeaderFormat_continuesChain() throws Exception {
    ...
    @Test void doFilter_invalidToken_sends401() throws Exception {
    ...
    @Test void doFilter_validToken_missingUserId_sends401() throws Exception {
    ...
    @Test void doFilter_validToken_userNotFound_sends401() throws Exception {
    ...
    @Test void doFilter_validToken_userBanned_sends403() throws Exception {
        UUID userId = UUID.randomUUID();
        JwtUserDetails details = new JwtUserDetails(userId, "email", UserRole.USER, Instant.now());
    ...
    @Test void doFilter_success_setsAuthentication() throws Exception {
        UUID userId = UUID.randomUUID();
        JwtUserDetails details = new JwtUserDetails(userId, "email", UserRole.USER, Instant.now());
        UserAccount account = new UserAccount(userId, "email", "pass", UserRole.USER);
