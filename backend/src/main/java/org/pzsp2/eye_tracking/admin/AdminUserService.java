...
public class AdminUserService {
  ...
  public List<AdminUserResponse> getAllOtherUsers(UUID currentUserId) {
    return userAccountRepository.findAll().stream()
        .filter(account -> currentUserId == null || !account.getUserId().equals(currentUserId))
        .map(this::toResponse)
        .toList();
  }

  @Transactional
  public AdminUserResponse updateRole(UUID userId, UserRole role) {
  ...
  @Transactional
  public AdminUserResponse updateBanned(UUID userId, boolean banned) {
    UserAccount account = loadUser(userId);
