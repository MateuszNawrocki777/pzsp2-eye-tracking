...
@Component class AdminUserInitializer implements ApplicationRunner {
    ...
    AdminUserInitializer(AdminUserProperties adminUserProperties,
                    UserAccountRepository userAccountRepository, PasswordService passwordService) {
    ...
    @Transactional public void run(ApplicationArguments args) {
        ...
        userAccountRepository.findByEmailIgnoreCase(email).ifPresentOrElse(existing -> {
            if (existing.getRole() != UserRole.ADMIN) {
                LOGGER.warn("Configured admin email {} already exists with role {}. Skipping auto-setup.",
                                email, existing.getRole());
                return;
            }

            if (!passwordService.matches(adminUserProperties.password(),
